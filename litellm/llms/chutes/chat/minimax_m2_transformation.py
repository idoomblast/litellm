"""
Transformation for MiniMax M2.1 model on Chutes platform.

MiniMax M2.1 uses native XML format for tool calls that needs to be transformed
...
from .transformation import ChutesChatConfig
...
class MiniMaxM2Config(ChutesChatConfig):
    """
    Config class for MiniMax M2.1 model on Chutes.
    
    Handles transformation of native XML tool calls to OpenAI format.
    """

    @classmethod
    def is_minimax_m2_model(cls, model: str) -> bool:
        """
        Detect if the model is a MiniMax M2.1 model.
        """
        return "minimax-m2" in model.lower() or "minimax_m2" in model.lower()
    ...
    def transform_response(
        ...
        """
        Transform the response from Chutes/MiniMax API.
