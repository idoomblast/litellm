...
class TestAlibabaCodingStreamingHandlerInheritance:
    ...
    def test_should_be_subclass_of_chutes(self):
        assert issubclass(
            AlibabaCodingStreamingHandler, ChutesChatCompletionStreamingHandler
        )
...
class TestRegularContentStreaming:
    ...
    def test_should_stream_regular_content(self):
        ...
        result = handler.chunk_parser(
            _make_chunk(role="assistant", content="Hello, world!")
        )
    ...
    def test_should_preserve_chunk_metadata(self):
        handler = _make_handler()
        result = handler.chunk_parser(
            _make_chunk(content="Test", chunk_id="chatcmpl-abc", model="kimi-k2.5-test")
        )
